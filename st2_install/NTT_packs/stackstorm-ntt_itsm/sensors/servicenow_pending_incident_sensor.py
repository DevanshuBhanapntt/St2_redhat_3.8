...
class servicenowpendingtasksensor(PollingSensor):
    ...
    def setup(self):
        ...
        self.som_company_sys_id =  self._config['servicenow']['company_sys_id']
    ...
    def poll(self):
        ...
        sn_task_endpoint = '/api/now/table/sc_task?sysparm_query=active=true^state=-5'
        sn_task_endpoint = sn_task_endpoint + '^company.sys_id='+self.som_company_sys_id
        sn_task_endpoint = sn_task_endpoint + '^assigned_to.name=Automation Service'
        sn_task_endpoint = sn_task_endpoint + '^u_pending_end_date_time<javascript:gs.minutesAgoStart(-15)'
        sn_task_endpoint = sn_task_endpoint + '^u_state_reason=106'
        sn_task_endpoint = sn_task_endpoint + '^ORu_state_reason=100'
        sn_task_endpoint = sn_task_endpoint + '^ORshort_descriptionENDSWITHAUTO'
        sn_task_endpoint = sn_task_endpoint + '^ORshort_descriptionSTARTSWITHRemove%20Decommissioned%20Server'
        sn_task_endpoint = sn_task_endpoint + '^ORshort_descriptionLIKESOM%20-%20Disable%20Linux%20Account'
        sn_task_endpoint = sn_task_endpoint + '^ORshort_descriptionSTARTSWITHValidate%20CMA%20Console'
        sn_task_endpoint = sn_task_endpoint + '^ORrequest_item.cat_item.nameINMessaging,Global%20Enterprise%20Server%20Provisioning,InfoSec%20-%20Termination,Bulk%20Add%20/%20Change%20/%20Terminate%20User,Enterprise%20Server%20Provisioning%20-%20Leveraged,Enterprise%20Server%20Decommissioning%20-%20Leveraged,Server%20Decommissioning,Virtual%20Server%20Provisioning'
        sn_task_endpoint = sn_task_endpoint + '^ORrequest_item.cat_item.nameENDSWITHWFAUTO'
        sn_task_endpoint = sn_task_endpoint + '^ORdescriptionINDatabase%20Security%20Request,Term-Remove%20Palo%20Alto%20Account'   

...
class ServicenowProcessingTasksRemove(BaseAction):
    def __init__(self, config):
        """Creates a new Action given a StackStorm config object (kwargs works too)
        :param config: StackStorm configuration object for the pack
        :returns: a new Action
        """
        super(ServicenowProcessingTasksRemove, self).__init__(config)

    def run(self, task_number, inc_st2_key):
        ...
        if task_number in processing_incs:
            processing_incs.remove(task_number)
            ...
            result = "The given Task number was not found in the list"

...
class helixprocessingincs_check(BaseAction):
    def __init__(self, config):
        """Creates a new Action given a StackStorm config object (kwargs works too)
        :param config: StackStorm configuration object for the pack
        :returns: a new Action
        """
        super(helixprocessingincs_check, self).__init__(config) 
    ...
    def run(self, inc_st2_key):
        ...
        for inc_id in processing_incs[:]:
            ...
            endpoint = '/api/arsys/v1/entry/HPD:IncidentInterface?q=%27Incident+Number%27%3D%22'+inc_id+'%22'  
            helix_inc_url = "https://{0}{1}".format(helix_url,endpoint)
            ...
            if (value["entries"][0]["values"]["Status"] == 'Resolved' or value["entries"][0]["values"]["Status"] == 'Closed' ) :

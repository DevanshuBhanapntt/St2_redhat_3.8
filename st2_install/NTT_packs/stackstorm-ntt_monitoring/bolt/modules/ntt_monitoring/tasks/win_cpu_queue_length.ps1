...
#This script gathers information relevant to diagnosing high CPU Utilization issues over RPC
#  1. Verifies the server pings, or reports probable cause of ping failure
#  2. Verifies the server supplies CPU utilization taken per SampleInterval and MaxSample variables, or reports probable cause of collection failure
#  3. Reports general information on boot time, memory utilization, and per CPU utilization; or reports probable cause of collection failure
#  4. Reports on highest utilization process and CPU usage of processes set in $TopProcessLimit, if first 3 items successful
#  5. IF svchost process(es) in $TopProcessLimit, lists services associated with each
#  6. IF w3wp process(es) in $TopProcessLimit, lists IIS web app pools associated with each

$top_process_limit=10
$SampleInterval=2
$MaxSamples=5
 
function get-UserTime($ServerName,$cpu_name) {
  #Determine CPU User Time Usage
  $Samples=get-counter -counter "\Processor($cpu_name)\% User Time" -ComputerName $ServerName -ErrorVariable errortext -ErrorAction SilentlyContinue
  if ($?)
  {
    ...
    "(\Processor($cpu_name)\% User time)`n"
  }
}

function get-QueueLength($ServerName,$cpu_name) {
  #Determine Average Queue Length
  $Samples=get-counter -counter "\System\Processor Queue Length" -ComputerName $ServerName -ErrorVariable errortext -ErrorAction SilentlyContinue
  if ($?)
  {
    # There should only be one sample returned but it gets returned in a array
    $samplecount=0
    ...
    $totalaverage=$totalusage/$samplecount
    write-output "System Processor Queue Length% : $totalaverage on $ServerName"     
  }
}
...
try
{
  ...
  $Samples=get-counter -counter "\Processor($cpu_name)\% processor time" -ComputerName $ServerName -ErrorVariable errortext -ErrorAction SilentlyContinue
  if ($?)
  {
    # There should only be one sample returned but it gets returned in a array
    $samplecount=0
    ...
    $totalaverage=$totalusage/$samplecount
    write-output "CPU% Utilization : $totalaverage% "
    ...
    #collect additional diagnostic data if WMI works
    {
      ...
      $proc=Get-counter -ComputerName $ServerName "\Process(*)\% processor time" -ErrorAction SilentlyContinue -ErrorVariable countererror
      ...
      foreach ($f in $procresult)
      {
        ...
        ### Forcibly suppress errors from process lookups, will fail if a process terminates during lookups
        $ErrorActionPreference = 'SilentlyContinue'
        $id = Get-Counter "\Process($ProcessName)\ID Process" -ComputerName $ServerName -ErrorAction SilentlyContinue

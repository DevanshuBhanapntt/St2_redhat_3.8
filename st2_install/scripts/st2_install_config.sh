...
is_rhel() {
  return $(cat /etc/os-release | grep -q 'ID="rhel"')
}
...
install_st2_pack() {
  sudo cp -r ../packs/st2/stackstorm-st2 /opt/stackstorm/packs/st2
  sudo cp -r ../packs/st2/virtualenv/st2 /opt/stackstorm/virtualenvs/st2
  sudo chgrp -R st2packs /opt/stackstorm/packs/st2
  sudo chgrp -R st2packs /opt/stackstorm/virtualenvs/st2
  sudo st2 pack register /opt/stackstorm/packs/st2
}
...
install_st2_dependencies() {
  ...
  if [[ -z "$is_epel_installed" ]]; then
    ...
    echo "Failed to retrieve epel-release" 1>&2
    return 2
  fi

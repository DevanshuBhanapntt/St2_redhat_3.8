...
finalize_yum_repo ()
{
  if [ "$_skip_pygpgme" = 0 ]; then
    ...
    yum install -y pygpgme --disablerepo='StackStorm_stable'
    ...
    if [ "$?" != "0" ]; then
      ...
      sed -i'' 's/repo_gpgcheck=1/repo_gpgcheck=0/' /etc/yum.repos.d/StackStorm_stable.repo
    fi
  fi

  echo "Installing yum-utils..."
  yum install -y yum-utils --disablerepo='StackStorm_stable'
  ...
  echo "Generating yum cache for StackStorm_stable..."
  yum -q makecache -y --disablerepo='*' --enablerepo='StackStorm_stable'

  echo "Generating yum cache for StackStorm_stable-source..."
  yum -q makecache -y --disablerepo='*' --enablerepo='StackStorm_stable-source'
}

finalize_zypper_repo ()
{
  zypper --gpg-auto-import-keys refresh StackStorm_stable
  zypper --gpg-auto-import-keys refresh StackStorm_stable-source
}
...
main ()
{
  ...
  yum_repo_config_url="https://packagecloud.io/install/repositories/StackStorm/stable/config_file.repo?os=${os}&dist=${dist}&source=script"
  # https://packagecloud.io/install/repositories/StackStorm/stable/config_file.repo?os=redhatenterpriseserver&dist=8&source=script

  if [ "${os}" = "sles" ] || [ "${os}" = "opensuse" ]; then
    yum_repo_path=/etc/zypp/repos.d/StackStorm_stable.repo
  else
    yum_repo_path=/etc/yum.repos.d/StackStorm_stable.repo
  fi
